 ...
 *  </code>
 */
class Google_Service_Digitalassetlinks_Resource_Assetlinks extends Google_Service_Resource
{
  /**
   ...
   * preconditions for an action.  For example, a client may want to know if it is
   * OK to send a web URL to a particular mobile app instead. The client can check
   * for the relevant asset link from the website to the mobile app to decide if
   * the operation should be allowed.
   *
   * A note about security: if you specify a secure asset as the source, such as
   * an HTTPS website or an Android app, the API will ensure that any statements
   * used to generate the response have been made in a secure way by the owner of
   * that asset.  Conversely, if the source asset is an insecure HTTP website
   * (that is, the URL starts with `http://` instead of `https://`), the API
   * cannot verify its statements securely, and it is not possible to ensure that
   * the website's statements have not been altered by a third party.  For more
   ...
   * 44:E5`.
   *
   * If these tools are not available to you, you can convert the PEM certificate
   ...
   * @opt_param string relation Query string for the relation.
   *
   * We identify relations with strings of the format `/`, where `` must be one of
   * a set of pre-defined purpose categories, and `` is a free-form lowercase
  ...
  public function check($optParams = array())
  {
    ...
    return $this->call('check', array($params), "Google_Service_Digitalassetlinks_CheckResponse");
  }
}
