...
class SendStickerTest extends TestCase
{
    public function testReplySticker()
    {
        $mock = function ($testRunner, $httpMethod, $url, $data) {
            ...
            $testRunner->assertEquals(MessageType::STICKER, $data['messages'][0]['type']);
            $testRunner->assertEquals('1', $data['messages'][0]['packageId']);
            $testRunner->assertEquals('2', $data['messages'][0]['stickerId']);

            return ['status' => 200];
        };
        $bot = new LINEBot(new DummyHttpClient($this, $mock), ['channelSecret' => 'CHANNEL-SECRET']);
        $res = $bot->replyMessage('REPLY-TOKEN', new StickerMessageBuilder('1', '2'));
    ...
    public function testPushSticker()
    {
        ...
        $bot = new LINEBot(new DummyHttpClient($this, $mock), ['channelSecret' => 'CHANNEL-SECRET']);
        $res = $bot->pushMessage('DESTINATION', new StickerMessageBuilder('1', '2'));
