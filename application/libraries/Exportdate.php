...
class Exportdate {
	...
	public function excel()	{
		...
		$this->CI->load->model('Export_model');

		$SelectData = [
			'fields' => ['export.*','ext.name AS type_name'],
			'where'  => ['export.status' => '0'],
		];
		$ExportList = $this->CI->Export_model->get_list($SelectData);
		...
		if (sizeof($ExportList) > 0) {
			foreach ($ExportList as $key => $value) {
				...
				$this->CI->Export_model->updata(['status' => '1'],['id' => $value->id]);
			}
		} else {
			//echo "Not Fount!!";
		}
...
     */
	public function SaveExcel($data) {
		...
		header('Content-Disposition: attachment;filename="{$this->ExcelName}.xls"');
		header('Cache-Control: max-age=0');
		$objWriter->save('php://output');
		//$objWriter->save("./uploads/excel/{$this->ExcelName}.xlsx");
		// return true;
	}

	public function AdvisoryExcel($data) {
		$this->CI->load->model('Advisory_model');
		...
		$return['data']  = $this->CI->Advisory_model->get_list($SelectData);
