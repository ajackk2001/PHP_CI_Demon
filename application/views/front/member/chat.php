...
<body>
    ...
    <script>
        ...
        var row = 1;//預設產出資料筆數
        var room_key='';
        var ajax_url = '/member/chat_json';
        var ajax_url2 = '/chat_get_json';
        var member_list=[];
        var name = '<?=$name;?>';
            ...
            $.ajax({
                ...
                success:function(data){//<img src="${(e.img||'/assets/images/front/member_default.jpg')}">
                            ...
                            <li>
                                <a href="javascript:;" class="chat_add" data-id="${e.to_user}" data-key="${e.room_key}">
                                    <div class="Messenger_pic">
                                            <span data-id="${e.to_user}" class="badge badge-danger">${data.read_num[e.to_user]>0?data.read_num[e.to_user]:''}</span>
                    ...
                    if(to_member){
                        ...
                        setTimeout(function(){console.log(document.getElementById('chatmessage').scrollHeight);scrollToBottom();}, 500);
            ...
            $.LoadTable2=function(key){
                $.ajax({
                    ...
                    success:function(data){//<img src="${(e.img||'/assets/images/front/member_default.jpg')}">
                        ...
                        room_key=to_member.room_key;
                        page=data.total+1;
                        $('span[data-id='+to_member.to_user+']').html('')
                        $('.read_num').html(data.chat_list_quantitye);
                        ...
                        success:function(data){//<img src="${(e.img||'/assets/images/front/member_default.jpg')}">
                            if(data.list[0]){
                               // var umsg=replace_em(umsg);//QQ表情 字串轉換
                                if(to_member_id==data.list[0].from_user){
