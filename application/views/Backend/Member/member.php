...
<html>
    ...
    <body class="fixed-left popoverLg">
                                            ...
                                            <table id="tech-companies-1" class="table table-striped table-bordered member_list">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th width="100" class="text-center" data-priority="1">聊天紀錄</th>
                                                        <th width="100" class="text-center" data-priority="1">目前點數</th>
                                                        <th width="100" class="text-center" data-priority="1">訂單累積金額(NTD)</th>
                                                        <th width="100" class="text-center" data-priority="1">已購買作品數量</th>
                                                        <th data-tablesaw-priority="4" width="100" class="text-center" data-priority="1"> <a data-toggle="tooltip" data-placement="left" title="" href="javascript:;" data-original-title="若要將會員設為黑名單，則不啟用此帳號，此會員將不能進行登入之動作">啟用</a></th>
        ...
        <script>
            ...
            var member_ajax =   '/Backend/Member/Itemlist';
                            ...
                            str +=`
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <select name="type" class="form-control text-center type" data-id="${e.id}" style="width: 120px;">
                                            <option value="1" ${e.type==1?'selected':''}>一般會員</option>
                                            <option value="2" ${e.type==2?'selected':''}>Girl</option>
                                            <option value="3" ${e.type==3?'selected':''}>創作者</option>
                                        </select>
                                    </td>
                                    ...
                                    <td class="text-center"><a href="/admin/chat/${e.id}" target="_blank"  data-toggle="tooltip" data-animation="false" data-placement="left" title="檢視該會員聊天紀錄"><i class="zmdi zmdi-comment-more text-info"></i></a></td>
                                    <td class="text-center"><a href="/Backend/Points?member_id=${e.id}" target="_blank"  data-toggle="tooltip" data-animation="false" data-placement="left" title="檢視該會員點數紀錄">${data.pointsTotal[e.id]}</a></td>
                                    <td class="text-center"><a href="/Backend/Orders/Item?member_id=${e.id}" target="_blank"  data-toggle="tooltip" data-animation="false" data-placement="left" title="檢視該會員訂單紀錄">$ ${data.paymentTotal[e.id]}</a></td>
                                    <td class="text-center"><a href="/Backend/Item/Item/${e.id}" target="_blank"  data-toggle="tooltip" data-animation="false" data-placement="left" title="檢視該會員已購買作品">${data.itemTotal[e.id]}</a></td>
                                    <td class="text-center"><input class="status" type="checkbox" ${(e.status==1?'checked':'')} data-id="${e.id}" data-plugin="switchery" data-color="#64b0f2" data-size="small" data-val="status"></td>
            ...
            var status_ajx  =   '/Backend/Member/Edit2/';
            $(document).on('change','.status',function(){
                var data={'status':(($(this).prop('checked'))?1:0)};
                if($(this).data('val')=='shop')data={'shop':(($(this).prop('checked'))?1:0)};
                if($(this).data('val')=='advanced_status')data={'advanced_status':(($(this).prop('checked'))?1:2)};
                $.ajax({
                    url:status_ajx+$(this).data('id'),
                    type:'POST',
                    dataType:'JSON',
                    data:data,
                    success:function(result){
