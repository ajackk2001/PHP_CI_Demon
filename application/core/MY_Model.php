...
class MY_Model extends CI_Model {
    ...
    public function insert_record($id){
        if($this->record){
            $data   =   [
                            ...
                            'action'        =>  '['.$this->trans_action[$this->action].']'.((isset($this->title[0]->title))?$this->title[0]->title:$this->title).$this->RecordHandle($id),
                        ];
     ...
     * 點閱率 COUNT
     */
    public function updateCount($where = [],$set = ['ctr'=>'ctr+1'])
    {
    ...
    //計算已使用類別
    public function getQuantity($str='type_id'){
        $data=$this->db->get($this->table_name)->result_array();
        $num=array();
        foreach ($data as $key => $v) {
            if(empty($num[$v[$str]]))$num[$v[$str]]=0;
            $num[$v[$str]]++;
        }
        return $num;
    }
