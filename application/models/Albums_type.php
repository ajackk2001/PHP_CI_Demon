...
class Albums_type extends My_Model{

    public $table_name	=	'albums_type';
    public $title	=	'花絮分類';
    ...
    public function search(array $select=['*'], array $conditions=[], array $order_by=[], array $limit=[]){
        $this->db->join('(select type_id,count(id) counts from albums group by type_id) total','albums_type.id=total.type_id','left');
