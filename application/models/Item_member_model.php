...
class Item_member_model extends My_Model{

    public $table_name	=	'item_member';
    ...
    public function search(array $select=['*'], array $conditions=[], array $order_by=[], array $limit=[]){
        ...
        if(preg_grep('/item_scale\./', $select))
            $this->db->join('item_scale', 'item_scale.id=item.scale_id', 'left');
        if(preg_grep('/item_type\./', $select))
            $this->db->join('item_type', 'item_type.id=item.type_id', 'left');
