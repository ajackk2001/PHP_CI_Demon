...
class Give_away_point_log_model extends My_Model{

	public $table_name	=	'give_away_point_log';
    public $title   =   '每日簽到紀錄';
    ...
    public function search(array $select=['*'], array $conditions=[], array $order_by=[], array $limit=[]){
        ...
        if(preg_grep('/give_away_point\./', $select))
            $this->db->join('give_away_point', 'give_away_point.id='.$this->table_name.'.give_away_point_id', 'left');
