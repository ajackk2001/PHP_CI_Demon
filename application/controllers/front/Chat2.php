...
class Chat2 extends MY_Front {
	...
	public function __construct(){
        ...
        if(!$this->session->Manager->id)redirect('/Backend/Logout');
	}
    //聊天管理
    public function chat($id=""){
        ...
        $data['member_id'] = $id ;
        $member    =  $this->member->get($id,['member.*']);
        $data['name'] = $member[0]->nickname;

        //載入模版
        $this->load->view('front/member/chat2',$data);
    }
    //聊天管理-聯絡人列表
    public function chat_json($id=''){
        $this->load->model('Chat_meta_model','chat_meta');
        $this->load->model('Chat_list_model','chat_list');
        ...
        if(!empty($return['list'])){
            ...
            //$return['chat_meta']=$this->db->last_query();
        ...
        try {
            ...
            if($member->type==2&&($this->points_total-$member->chat_point)>=0){
                ...
                $cashdata2=[
                    ...
                    'member_id'=>$member->id,
                    'USD'=>round(($member->chat_point/2)/$set->points,1),
                    'date_add'=>date("Y-m-d H:i:s"),
                    'type'=>3,
                    'payment_sn'=>'',
                    'item_id'=>'',
    ...
    public function chat_get_json($id=''){
        $select = ['chat_list.*'];
        $orderby    =   ['update_time'];
        $conditions = [];//
        $conditions[] = [
        ...
        $return['to_member'] = $this->chat_meta->get(['from_user'=>$id,'room_key'=>$this->input->post('room_key')],['chat_meta.*','to_member.chat_point','to_member.nickname','to_member.type','CONCAT("/",to_member.img) as img'],['update_time asc'],['limit'=>1])[0];
