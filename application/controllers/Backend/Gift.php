...
class Gift extends MY_Manager {
    public $upload_setting  =   [
        'upload_path'   =>  '/uploads/gift/',
        'allowed_types' =>  ['jpg', 'jpeg', 'png', 'gif'],
        'max_size'       =>  8,
    ];
    private $img_dir = 'uploads/gift/';
    ...
    **/
	public function Index(){
        ...
        $data['img_width']="500";
        $data['img_height']="500";
		$this->load->view('Backend/Gift/itemlist',$data);
	}
    ...
    **/
    public function Show(){
        ...
        $return['list'] 	=	$this->gift->search(['gift.*'],$conditions,$orderby,$limit);
        $return['total']    =   $this->gift->row_count;
    ...
    **/
    public function Edit($id=''){
        ...
        try {
            ...
            $where  =   ['id' =>  $id];
            $result = $this->gift->update($data,$where);
            if (!$result)
    ...
    **/
    public function Delete($id=''){
        try{
            $original = $this->gift->get(['id'=>$id]);
            if (empty($id) || count($original) == 0)
                throw new Exception('查無資料');

            $where = ['id' => $id];
            $result = $this->gift->delete($where);
            if (!$result)
    ...
    **/
    public function UpdateSeq(){
        $data   =   $this->input->post();
        $result     =   $this->gift->UpdateSeq($data['seq']);
        if ($result){
    ...
    **/
    public function lastSeq(){
        $lastSeq = $this->gift->search(['max(seq) MaxSeq']);
