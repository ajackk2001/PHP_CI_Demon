...
class Banner_img extends MY_Manager {
    ...
    **/
	public function Index(){
        $data['img_width']="472";
        $data['img_height']="314";
		$this->load->view('Backend/Banner/banner_img',$data);
	}
    ...
    **/
    public function Show(){
        ...
        $return['list'] 	=	$this->banner_img->search(['id','update_time','title','CONCAT("/",img) img','weblink','seq','status'],$conditions,$orderby,$limit);
        $return['total']    =   $this->banner_img->row_count;
    ...
    **/
    public function Add(){
        ...
        try {
            ...
            $result = $this->banner_img->insert($data);
            if (!$result)
    ...
    **/
    public function Edit($id=''){
        include 'assets/plugins/slim/server/slim.php';
        try {
            $original = $this->banner_img->get($id);
            ...
            $where  =   ['id' =>  $id];
            $result = $this->banner_img->update($data,$where);
            if (!$result)
    ...
    **/
    public function Delete($id=''){
        try{
            $original = $this->banner_img->get(['id'=>$id]);
            if (empty($id) || count($original) == 0)
                throw new Exception('查無資料');

            $where = ['id' => $id];
            $result = $this->banner_img->delete($where);
            if (!$result)
    ...
    **/
    public function Info($id=''){
        include 'assets/plugins/slim/server/slim.php';
        $return     =  $this->banner_img->get($id,['id','title','img','weblink','seq']);
    ...
    **/
    public function UpdateSeq(){
        $data   =   $this->input->post();
        $result     =   $this->banner_img->UpdateSeq($data['seq']);
        if ($result){
    ...
    **/
    public function lastSeq(){
        $lastSeq = $this->banner_img->search(['max(seq) MaxSeq']);
