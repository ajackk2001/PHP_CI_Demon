...
class Relations_website extends MY_Manager {
    public $upload_setting  =   [
        'upload_path'   =>  '/uploads/relations_website/',
        'allowed_types' =>  ['jpg', 'jpeg', 'png', 'gif'],
        'max_size'       =>  8,    
    ];
    private $img_dir = 'uploads/relations_website/';

	public function __construct(){
		parent::__construct();
		$this->load->library('form_validation');
		$this->load->model('Relations_website_model','Relations_website');
    }
    ...
    **/
	public function Index(){
		$this->load->view('Backend/Relations/Relations_website');
	}
    ...
    **/
    public function Show(){
        ...
        $orderby = ['seq ASC','publish_time DESC','id DESC'];
        $conditions = [];
        ...
        $return['list'] 	=	$this->Relations_website->search(['id','title','CONCAT("/",logo) logo','weblink','seq','status','ctr'],$conditions,$orderby,$limit);
        $return['total']    =   $this->Relations_website->row_count;
    ...
    public function Add(){
        ...
        try {
            ...
            if (isset($image['output']['data'])) {
                ...
                $data['logo'] = $output['path'];
                unset($data['slim']);
            }

            $result = $this->Relations_website->insert($data);
            if (!$result)
    ...
    **/
    public function Edit($id=''){
        include 'assets/plugins/slim/server/slim.php';
        try {
            $original = $this->Relations_website->get($id);
            ...
            if (isset($image['output']['data'])) {
                $slimName = $image['output']['name'];
                $slimData = $image['output']['data'];
                $slimUnique = ($this->img_dir.$slimName == $original[0]->logo)? false:true;
                $output = Slim::saveFile($slimData, $slimName, $this->img_dir, $slimUnique);

                $data['logo'] = $output['path'];
                unset($data['slim']);

                if ($slimUnique && file_exists($original[0]->logo)){
                    @unlink($original[0]->logo);
                }
            }

            $where  =   ['id' =>  $id];
            $result = $this->Relations_website->update($data,$where);
            if (!$result)
    ...
    **/
    public function Delete($id=''){
        try{
            $original = $this->Relations_website->get(['id'=>$id]);
            if (empty($id) || count($original) == 0)
                throw new Exception('查無資料');

            $where = ['id' => $id];
            $result = $this->Relations_website->delete($where);
            if (!$result)
            ...
            foreach ($original[0] as $key=>$val){
                if (in_array($key,['logo']) && !empty($val)){
                    @unlink($val);
                }
            }
    ...
    **/
    public function Info($id=''){
        include 'assets/plugins/slim/server/slim.php';
        $return     =  $this->Relations_website->get($id,['id','title','logo','weblink','seq']);
        if (empty($return)) redirect('/Backend/RelationsWebsite');
        $return = current($return);
        $return->logo = site_url($return->logo);
        $this->output->set_content_type('application/json')->set_output(json_encode($return));
    }
    ...
    **/
    public function UpdateSeq(){
        $data   =   $this->input->post();
        $result     =   $this->Relations_website->UpdateSeq($data['seq']);
        if ($result){
    ...
    **/
    public function lastSeq(){
        $lastSeq = $this->Relations_website->search(['max(seq) MaxSeq']);
