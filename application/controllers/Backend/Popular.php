...
class Popular extends MY_Manager {
    ...
    **/
	public function Index(){
        ...
        $data['img_width']="180";
        $data['img_height']="180";
		$this->load->view('Backend/Popular/itemlist',$data);
	}
    ...
    **/
    public function Show(){
        ...
        $return['list'] 	=	$this->popular->search(['popular.*','CONCAT("/",img) img','item_category.title as category_title','item_type.title as type_title'],$conditions,$orderby,$limit);
        $return['total']    =   $this->popular->row_count;
    ...
    **/
    public function Add(){
        ...
        try {
            ...
            $result = $this->popular->insert($data);
            if (!$result)
    ...
    **/
    public function Edit($id=''){
        ...
        try {
            ...
            $where  =   ['id' =>  $id];
            $result = $this->popular->update($data,$where);
            if (!$result)
    ...
    **/
    public function Delete($id=''){
        try{
            $original = $this->popular->get(['id'=>$id]);
            if (empty($id) || count($original) == 0)
                throw new Exception('查無資料');

            $where = ['id' => $id];
            $result = $this->popular->delete($where);
            if (!$result)
    ...
    **/
    public function Info($id=''){
        include 'assets/plugins/slim/server/slim.php';
        $return     =  $this->popular->get($id,['popular.*']);
    ...
    **/
    public function UpdateSeq(){
        $data   =   $this->input->post();
        $result     =   $this->popular->UpdateSeq($data['seq']);
        if ($result){
    ...
    **/
    public function lastSeq(){
        $lastSeq = $this->popular->search(['max(seq) MaxSeq']);
