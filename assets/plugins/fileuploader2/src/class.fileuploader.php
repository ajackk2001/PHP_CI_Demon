...
class MyFileUploader {
	...
	public function __construct($name, $options = null) {
        ...
        $this->CI->load->library('upload');     //檔案上傳類庫
		$this->CI->load->library('image_lib');  //圖片處理類庫
	...
	private function editFiles() {
		...
		foreach($files as $key=>$file) {
			...
			// add editor to preloaded files if available
			...
			// edit file
			if ($this->options['editor'] != null && isset($file['editor']) && file_exists($file['file']) && strpos($file['type'], 'image/') === 0) {
	...
	private function sortFiles(&$data = null) {
		...
		foreach($files as $key=>$file) {
			...
			// add index to preloaded files if available
